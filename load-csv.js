...
function loadCSV(
  ...
  data = data.map((row) => _.dropRightWhile(row, (val) => val === "")); //dropout trailing columns(commas)
  const headers = _.first(data);
  ...
  data = data.map((row, index) => {
    ...
    return row.map((element, index) => {
      if (converters[headers[index]]) {
        const converted = converters[headers[index]](element); //custom value parsing
        return _.isNaN(converted) ? element : converted;
      }

      const result = parseFloat(element);
      return _.isNaN(result) ? element : result; //parsing number values
    });
  });

  let labels = extractColumns(data, labelColumns); //extracting separated columns
